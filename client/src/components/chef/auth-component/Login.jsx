...
import { setChefToken, removeChefToken,chefDetails,rem_chefDetails } from "@/redux/adminSlice/authSlice";
...
const ChefLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post("/api/auth/chefLogin", loginDetails, {
        headers: {
      ...
      if (res?.data?.success) {
        ...
        setLoading(false);
        dispatch(setChefToken(res?.data?.token));
        dispatch(chefDetails(res?.data?.user));
              ...
              <form action="" className="" onSubmit={handleSubmit}>
                <div className="flex flex-col gap-4 justify-center p-8 lg:p-14 md:max-w-[80%] lg:w-full lg:max-w-[100%] mx-auto ">
                  <div className="text-left ">
                    <p className="mb-2 bold-40 text-primary">Welcome Chef</p>
