...
exports.getCartItems = async (req, res) => {
  try {
    ...
    res.status(200).json({ cart: userCart, TotalPrice: TotalPricePerQuntity });
  } catch (error) {
...
exports.deleteCartItem = async (req, res) => {
  try {
    ...
    // Respond with a success message
    res.status(200).json({ message: "Item deleted successfully", cart: userCart });
  } catch (error) {
    // If an error occurs, respond with an error message
    res.status(500).json({ error: "Failed to delete item from cart", message: error.message });
  }
};
